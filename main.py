...
#  self.channel_name = input("Enter channel_name:\t")
 ...
 def authenticate(self):
    ...
    if os.path.exists('token.json'):
      creds = Credentials.from_authorized_user_file('token.json', self.self.SCOPES)
 ...
 def authenticate1(self):
    ...
    if os.path.exists('token1.json'):
      creds = Credentials.from_authorized_user_file('token1.json', self.self.SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
 ...
 def authenticate2(self):
    ...
    if os.path.exists('token2.json'):
      creds = Credentials.from_authorized_user_file('token2.json', self.self.SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
 ...
 def authenticate3(self):
    ...
    if os.path.exists('token3.json'):
      creds = Credentials.from_authorized_user_file('token3.json', self.self.SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file(
                'credentials.json', self.SCOPES)
            creds = flow.run_local_server(port=0)
        with open('token.json3', 'w') as token:
            token.write(creds.to_json())
    return build(self.API_SERVICE_NAME, self.API_VERSION, credentials=creds)
 def authenticate4(self):
    creds = None
    if os.path.exists('token4.json'):
      creds = Credentials.from_authorized_user_file('token4.json', self.self.SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
 ...
 def authenticate5(self):
    ...
    if os.path.exists('token5.json'):
      creds = Credentials.from_authorized_user_file('token5.json', self.self.SCOPES)
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file(
                'credentials.json', self.SCOPES)
            creds = flow.run_local_server(port=0)
        with open('token.json5', 'w') as token:
            token.write(creds.to_json())
    return build(self.API_SERVICE_NAME, self.API_VERSION, credentials=creds)
 def logo(self):
  print (f"""
{os.system('figlet -c -f banner Youtube | lolcat')}\n{os.system('figlet -c -f banner Commenter | lolcat')}
 ...
 def reply_to_comment(self, video_id, comment_id, comment_text):
    try:
        ...
        print(f"replied to comment {top_level_comment_id} with reply id {reply_id}")
    except HttpError as error:
        print('An error occurred: %s' % error)
 ...
 def run(self):
  ...
  if __name__ == '__main__':
    ...
    self.service2 = self.authenticate3()
  ...
  self.reply_to_comment(video_id, comment_id, comment_text)
  print(f"\n[+] {comment_info}")
  print(f"\n[-] {link}\n")
